...
class OrchestratorDashboardService:
    ...
    async def get_task_trace(self, task_id: str) -> str | None:
        """Returns the redacted debug trace for a task, or None if the task or its
        trace is not found.

        Reads trace_json directly off the TaskRecord (like get_logs does for agent
        logs) rather than through to_dict(), which deliberately excludes it.
        """
        task = await self.tasks.get_by_id(task_id)
        return task.trace_json if task else None
