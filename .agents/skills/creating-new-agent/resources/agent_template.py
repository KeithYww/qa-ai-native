...
class <AgentName>Agent(AgentBase):
    ...
    def get_skills(self) -> list[AgentSkill]:
        # The description is also read by the orchestrator's task-routing discovery
        # prompt — keep it a real, capability-level summary, not a placeholder.
        return [
            AgentSkill(
                id="<agent-name-kebab-case>",
                name="<Short human-readable skill name>",
                description="<Brief description of what this agent does>",
                tags=["qa", "<agent-name-kebab-case>"],
            )
        ]
