...
``serverInfo`` on construction, then runs JQL searches against ``/rest/api/2/search``
(the RAG sync). Every search answers with the same seeded story the Jira MCP mock serves.
"""
...
async def recorded() -> dict:
    """No state to report anymore (nothing posts comments here); kept only as the
    container healthcheck target, matching the other mocks' convention."""
    return {}
