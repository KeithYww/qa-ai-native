...
@pytest.mark.asyncio
async def test_execute_emits_trace_artifact(mock_agent, mock_context, mock_event_queue):
    """When the agent captured a trace, a trace artifact is emitted before completion."""
    executor = DefaultAgentExecutor(mock_agent)
    mock_context.message = MagicMock(spec=Message)

    trace_bytes = b'[{"parts": []}]'

    # Mirror AgentBase.run, which populates latest_trace during the run (the executor
    # resets it to None at task start, so it must be set from within run()).
    async def run_and_capture_trace(_message):
        mock_agent.latest_trace = trace_bytes
        result = MagicMock()
        result.parts = []
        return result

    mock_agent.run.side_effect = run_and_capture_trace
    ...
    trace_artifacts = [
        call[0][0]
        for call in mock_event_queue.enqueue_event.call_args_list
        if isinstance(call[0][0], TaskArtifactUpdateEvent) and call[0][0].artifact.name == "agent_trace"
    ]
    assert len(trace_artifacts) == 1
    assert trace_artifacts[0].artifact.parts[0].raw == trace_bytes
