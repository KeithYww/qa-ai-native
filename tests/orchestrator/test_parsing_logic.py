...
# =============================================================================
# Tests for _extract_requirement_review_ref
# =============================================================================


class TestExtractRequirementReviewRef:
    """Tests for parsing the Feishu Project native WorkFlowNodeStatusEvent payload."""

    def test_extracts_all_fields_from_nested_payload(self):
        from orchestrator.main import _extract_requirement_review_ref

        body = {
            "payload": {
                "id": "WI-1",
                "project_simple_name": "PROJ",
                "nodes": [
                    {"node_form": [{"field_type_key": "text", "field_value": "irrelevant"}]},
                    {"node_form": [{"field_type_key": "link", "field_value": "https://example.feishu.cn/docx/x"}]},
                ],
            }
        }
        ...
        assert feishu_doc == "https://example.feishu.cn/docx/x"

    def test_falls_back_to_top_level_when_no_payload_key(self):
        from orchestrator.main import _extract_requirement_review_ref

        body = {
            "id": "WI-2",
            "project_simple_name": "PROJ2",
            "nodes": [{"node_form": [{"field_type_key": "link", "field_value": "https://example.feishu.cn/docx/y"}]}],
        }

        work_item_id, project_key, feishu_doc = _extract_requirement_review_ref(body)

        assert work_item_id == "WI-2"
        assert project_key == "PROJ2"
        assert feishu_doc == "https://example.feishu.cn/docx/y"

    def test_missing_project_key_falls_back_to_config_default(self):
        import config
        from orchestrator.main import _extract_requirement_review_ref

        body = {"payload": {"id": "WI-3", "nodes": []}}

        _, project_key, _ = _extract_requirement_review_ref(body)

        assert project_key == config.MEEGO_PROJECT_KEY

    def test_no_link_field_returns_empty_doc(self):
        from orchestrator.main import _extract_requirement_review_ref
        ...
        body = {
            "payload": {
                "id": "WI-4",
                "project_simple_name": "PROJ",
                "nodes": [{"node_form": [{"field_type_key": "text", "field_value": "not a link"}]}],
            }
        }

        _, _, feishu_doc = _extract_requirement_review_ref(body)
    ...
    def test_missing_id_returns_empty_string(self):
        ...
        work_item_id, _, _ = _extract_requirement_review_ref({"payload": {"nodes": []}})

        assert work_item_id == ""


# =============================================================================
# Tests for _RequirementReviewGuard
# =============================================================================


class TestRequirementReviewGuard:
    """Tests for the in-memory dedup + rate-limit guard behind /requirement-ready-for-review."""

    @pytest.mark.asyncio
    async def test_first_request_is_allowed(self):
        from orchestrator.main import _RequirementReviewGuard

        guard = _RequirementReviewGuard(dedup_window_seconds=300, global_max_per_minute=5)

        assert await guard.check("WI-1") is None

    @pytest.mark.asyncio
    async def test_duplicate_within_window_is_dropped(self):
        from orchestrator.main import _RequirementReviewGuard

        guard = _RequirementReviewGuard(dedup_window_seconds=300, global_max_per_minute=5)

        assert await guard.check("WI-1") is None
        assert await guard.check("WI-1") == "duplicate"

    @pytest.mark.asyncio
    async def test_duplicate_after_window_expires_is_allowed_again(self, monkeypatch):
        from orchestrator.main import _RequirementReviewGuard
        ...
        assert await guard.check("WI-1") is None
        clock[0] += 301  # past the dedup window, but also past the 60s rate window
        assert await guard.check("WI-1") is None

    @pytest.mark.asyncio
    async def test_global_rate_limit_drops_excess_distinct_requests(self, monkeypatch):
        from orchestrator.main import _RequirementReviewGuard

        guard = _RequirementReviewGuard(dedup_window_seconds=300, global_max_per_minute=2)
        clock = [1000.0]
        monkeypatch.setattr("time.monotonic", lambda: clock[0])

        assert await guard.check("WI-1") is None
        assert await guard.check("WI-2") is None
        assert await guard.check("WI-3") == "rate_limited"

    @pytest.mark.asyncio
    async def test_global_rate_limit_window_slides(self, monkeypatch):
        from orchestrator.main import _RequirementReviewGuard
        ...
        assert await guard.check("WI-1") is None
        assert await guard.check("WI-2") is None
        clock[0] += 61  # oldest entry falls out of the 60s window
        assert await guard.check("WI-3") is None

    @pytest.mark.asyncio
    async def test_rate_limit_checked_before_dedup(self, monkeypatch):
        """A rate-limited request must not be recorded as seen, so it isn't misreported as a duplicate later."""
        from orchestrator.main import _RequirementReviewGuard

        guard = _RequirementReviewGuard(dedup_window_seconds=300, global_max_per_minute=1)
        clock = [1000.0]
        monkeypatch.setattr("time.monotonic", lambda: clock[0])

        assert await guard.check("WI-1") is None
        assert await guard.check("WI-2") == "rate_limited"
        assert "WI-2" not in guard._last_seen

    @pytest.mark.asyncio
    async def test_concurrent_duplicate_requests_only_one_passes(self):
        """Two concurrent checks for the same work item must not both be allowed through."""
        import asyncio
        ...
        results = await asyncio.gather(guard.check("WI-1"), guard.check("WI-1"))

        assert sorted(str(r) for r in results) == ["None", "duplicate"]


# =============================================================================
# Tests for _run_requirement_review
# =============================================================================


class TestRunRequirementReview:
    """Tests that _run_requirement_review contains its own errors, mirroring _run_pipeline."""

    @pytest.mark.asyncio
    async def test_agent_failure_is_contained_not_raised(self, mock_error_history):
        """A failing agent call must be recorded, not propagated to the queue consumer."""
        from orchestrator.main import _run_requirement_review

        with patch("orchestrator.main._send_task_to_agent", new_callable=AsyncMock) as mock_send:
            mock_send.side_effect = RuntimeError("agent unreachable")

            await _run_requirement_review("WI-1", "PROJ", "https://example.feishu.cn/docx/x")  # must not raise

        mock_send.assert_called_once()

    @pytest.mark.asyncio
    async def test_success_validates_task_status(self, mock_error_history):
        from orchestrator.main import _run_requirement_review
        ...
        assert mock_validate.call_args.args[0] is mock_task
